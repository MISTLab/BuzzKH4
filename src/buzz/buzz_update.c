...
//TODO : change the Bo buffer to one from utility and update monitor
...
//fprintf(stdout,"[Debug : ]updater value = %i \n",updater->mode);
	if(*(int*)updater->mode==CODE_RUNNING){
		...
		if(check_update()){
			//TODO :change to something clean 
			//bzzc -b test.bo -d test.bdbg -a test.basm test.bzz
			char* bzzcomp="bzzc ";
			char* hyp_b="-b ";
			char* hyp_d=" -d ";
			char* hyp_a=" -a ";
			char* filenametmp=strdup(bcfname);//[strlen(bcfname)+2];
			//memcpy(filenametmp, bcfname, strlen(bcfname)+2);
   			char* lastdot = strrchr (filenametmp, '.');
	           	if (lastdot != NULL)
			   *lastdot = '\0';
			//remove_extension(filenametmp,bcfname);
			 //fprintf(stdout,
       			char* basmname= strcat(filenametmp,".basm ");
			//free(filenametmp);
			int i = 0;			
			i = strlen(bzzcomp)+strlen(hyp_b)+strlen(bcfname)+strlen(hyp_d)+strlen(dbgfname)+strlen(hyp_a)+strlen(basmname)+strlen(bzz_file);
                        char comp_total[i];
			//memset(comp_total, 0,sizeof(char)*i+1);
			comp_total[0]='\0';
			//TODO change to something better than strcat !!			
			strcat(comp_total, bzzcomp);   
  			strcat(comp_total, hyp_b);   
  			strcat(comp_total, bcfname);   
  			strcat(comp_total, hyp_d);   
			strcat(comp_total, dbgfname);   
    			strcat(comp_total, hyp_a);   
    			strcat(comp_total, basmname);
			
			strcat(comp_total, bzz_file);
 
			fprintf(stdout,"[debug]concatinated string\n");
			fprintf(stdout,comp_total);
			...
			if ((fp = popen(comp_total, "r")) == NULL) { // to change file edit
			fprintf(stdout,"Error opening pipe!\n");
		 	    }
		 	 while (fgets(buf, 128, fp) != NULL) {
				fprintf(stdout,"OUTPUT: %s \n", buf);
		...
		buzzvm_pushs(VM, buzzvm_string_register(VM, "barrier_val",1));
            	...
            	buzzvm_pop(VM);
		fprintf(stdout,"Barrier ..................... %i and no of robots %i \n",tObj->i.value,no_of_robot);
                 
		if(tObj->i.value==no_of_robot) { 
			fprintf(stdout,"setting code running\n");
			*(int*)(updater->mode) = CODE_RUNNING;
			//gettimeofday(&t2, NULL);
			//collect_data();
			timer_steps=0;
			neigh=0;
			fprintf(stdout,"setting update code\n");
			if(buzz_update_set((updater)->bcode, (char*)dbgfname, *(size_t*)(updater->bcode_size))) fprintf(stdout,"System updated sucessfully !!!\n");
			VM = get_vm();
...
int test_set_code(uint8_t* BO_BUF, const char* dbgfname,size_t bcode_size ){
	if(buzz_update_init_test(BO_BUF, dbgfname,bcode_size)){
		...
		if(update_step_test()){
			...
			*(size_t*)updater->bcode_size = bcode_size;
			buzz_update_set((updater)->standby_bcode,
		...
		/*Unable to step something wrong*/
			...
			buzz_update_set((updater)->bcode, dbgfname, (size_t)*(size_t*)(updater->bcode_size));
			}
			else{
			/*You will never reach here*/
			fprintf(stdout,"step test failed, Return to stand by\n");
			buzz_update_init_test((updater)->standby_bcode,
			         (char*)dbgfname,(size_t) *(size_t*)(updater->standby_bcode_size));
			buzzvm_t  VM = get_vm();
			buzzvm_pushs(VM, buzzvm_string_register(VM, "ROBOTS", 1));
			buzzvm_pushi(VM, no_of_robot);
		...
		/*TODO You will never reach here : think about roll back if updated dosn't converge*/
...
/*void remove_extension(char* dst, const char* filename) {
    size_t len = strdup(filename);
    memcpy(dst, filename, sizeof(char)*len);
    char* lastdot = strrchr (dst, '.');
    if (lastdot != NULL)
        *lastdot = '\0';
}*/
/*char* remove_extension (const char* str) {
    ...
    if ((retstr = malloc (strlen (str) + 2)) == NULL) return NULL;
    strcpy (retstr, str);
